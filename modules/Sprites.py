...
''' this class defines the bunny, it's position and the image of bunny.
    this is like blueprint to create as many bunny as we want '''
...
class BunnySprite(pygame.sprite.Sprite):
    ...
    'this method will move the human in specified direction and speed as instructed in main file'
...
'''blueprint for bullet is defined with argument of image and position'''
...
class bulletSprite(pygame.sprite.Sprite):
    ...
    def update(self, screensize):
        ...
        self.rect.left += velx
        self.rect.top += vely
        if self.rect.right < 0 or self.rect.left > screensize[0] or self.rect.top > screensize[
            1] or self.rect.bottom < 0:
            return True
        return False
...
class villainSprite(pygame.sprite.Sprite):
    def __init__(self, image, position):
        pygame.sprite.Sprite.__init__(self)
        self.image = image
        self.rect = self.image.get_rect()
